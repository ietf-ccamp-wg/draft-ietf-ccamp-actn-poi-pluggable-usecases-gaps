...
Current draft is applicable to programmable pluggables in general. By programmable we mean to include both coherent and non coherent pluggables. Depending on the technology of the pluggable there might be several parameters to configure. One configuration aspect is the ability to select the operational mode {{!?I-D.-ccamp-optical-impairment-topology-yang}} (in case several modes are supported), and, for a particular operational mode, the specific parameters to be specified (e.g. frequency, output power…. ) and read. Operational mode is described in IETF in  {{!?I-D.-ccamp-optical-impairment-topology-yang}} . ITU-T Rec G.698.2 refers to application codes, while CMIS refers to AppSel code (Application Selector). In Openconfig, it is referred as Operational mode.
